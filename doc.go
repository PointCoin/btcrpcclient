...
API.  This client has been tested with btcd (https://github.com/PointCoin/btcd),
wallet (https://github.com/PointCoin/wallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, btcd and
wallet provide a websocket interface that is more efficient than the standard
...
btcd or wallet by default.  However, configuration options are provided to
...
wallet only uses a single connection that remains open and allows
...
wallet passthrough).  Therefore if you call one of these commands against
...
functionality into a separate process named wallet.  This means if you are
...
need chain-related RPCs.  In contrast, wallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - btcdwebsockets
   ...
   block count
 - walletwebsockets
   Connects to a wallet RPC server using TLS-secured websockets, registers
